...
test('EngineCoordinator orchestrates engine lifecycle', async () => {
  ...
  assert(quantum === null, 'Non-initial engines should be instantiated lazily');
  ...
  assert(broadcastHandled === true, 'Broadcast should be handled by initialised engines');
  assert(faceted.customPayload.boost === 1, 'Broadcast payload should reach active engine');
  ...
  const lazyQuantum = coordinator.getEngine('quantum');
  assert(lazyQuantum instanceof StubEngine, 'Quantum engine should be initialised lazily');
  assert(lazyQuantum.active === true, 'Target engine should be active');
  ...
  assert(lazyQuantum.lastSize.width === 1920, 'Active engine should receive resize dimensions');

  const postSwitchBroadcast = coordinator.broadcast('customHook', { boost: 2 });
  assert(postSwitchBroadcast === true, 'Broadcast should continue to propagate to active engines');
  assert(lazyQuantum.customPayload.boost === 2, 'Broadcast should reach newly initialised engine');
