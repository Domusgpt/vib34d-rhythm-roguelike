...
export class EngineCoordinator {
  ...
  async initialize({ initialSystem = null } = {}) {
    ...
    this.initializationSequence = orderedEntries.map(([systemName]) => systemName);

    const preferredSystem = (initialSystem && this.engineClasses.has(initialSystem))
      ? initialSystem
      : this.initializationSequence.find((systemName) => this.engineClasses.has(systemName));

    if (preferredSystem) {
      await this.ensureEngine(preferredSystem);
    }
  }
  ...
  async ensureEngine(systemName) {
    ...
    if (!this.engineClasses.has(systemName)) {
      console.warn(`EngineCoordinator: no engine registered for ${systemName}`);
      return null;
    }

    if (this.pendingInitializations.has(systemName)) {
      return this.pendingInitializations.get(systemName);
    }
    ...
    this.pendingInitializations.set(systemName, promise);

    try {
      return await promise;
    } finally {
      this.pendingInitializations.delete(systemName);
    }
  }
  ...
  async switchEngine(targetSystemName) {
    ...
    try {
      const nextEngine = await this.ensureEngine(targetSystemName);
      if (!nextEngine) {
        throw new Error(`EngineCoordinator: failed to initialise ${targetSystemName}`);
      }
