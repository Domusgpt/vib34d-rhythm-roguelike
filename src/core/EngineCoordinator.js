...
export class EngineCoordinator {
  ...
  getEngineCanvasResources(systemName, config) {
    ...
    layers.forEach((layerRef, index) => {
      let resource = this.canvasPool.getCanvasResources(systemName, layerRef);
      if (!resource?.isValid) {
        resource = this.canvasPool.getCanvasResources(systemName, index);
      }

      if (!resource?.isValid) {
        throw new Error(`EngineCoordinator: invalid canvas resource for ${systemName} layer ${layerRef}`);
      }

      const key = typeof layerRef === 'string' ? layerRef : String(index);
