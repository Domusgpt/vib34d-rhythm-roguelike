...
export class EngineCoordinator {
  ...
  async initializeEngine(systemName) {
    ...
    const sharedResources = this.resolveSharedResources(systemName, config);
  ...
  resolveSharedResources(systemName, config) {
    ...
    Object.entries(config.resourceRequirements).forEach(([type, keys]) => {
      const registry = this.sharedResources.get(type);
      if (!registry) {
        return;
      }
      ...
      keys.forEach((key) => {
        if (!registry.has(key)) {
          console.warn(`EngineCoordinator: shared ${type} ${key} missing for ${systemName}`);
          return;
        }

        resolved[type][key] = registry.get(key);
      });
    });
